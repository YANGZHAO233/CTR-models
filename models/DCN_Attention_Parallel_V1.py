...
class Cross_layer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(self.layer_num):
            ...
            x0l = tf.math.exp(x0l)
            sum_x0l = tf.math.reduce_sum(x0l, axis=2, keepdims=True)
            alpha = tf.math.divide(x0l, sum_x0l)
            xl_w = tf.matmul(alpha, xl)
            xl = tf.math.multiply(x0_wv, xl_w) + self.cross_bias[i] + xl

        output = tf.squeeze(xl, axis=2)
        return output


class DCN_Attention_Parallel_V1(Model):
    def __init__(self,
